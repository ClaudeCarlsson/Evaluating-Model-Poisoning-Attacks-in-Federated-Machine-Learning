...
class PackageRuntime:
    ...
    def unpack(self):
        """
        Unpack the tarball package, excluding 'client/entrypoint' if it exists.
        """
        if self.pkg_name:
            f = None
            mode = ''
            if self.pkg_name.endswith('tar.gz') or self.pkg_name.endswith('.tgz'):
                mode = 'r:gz'
            elif self.pkg_name.endswith('tar.bz2'):
                mode = 'r:bz2'

            if mode:
                f = tarfile.open(os.path.join(self.pkg_path, self.pkg_name), mode)

            if not f:
                print("Failed to open the tarball file.")
                return
        ...
        try:
            os.chdir(self.dir)
            members_to_extract = [m for m in f.getmembers() if m.name != 'client/entrypoint']
