...
class PackageRuntime:
    ...
    def unpack(self):
        ...
        try:
            ...
            if f:
                # Get all file names in the archive, excluding 'fedn.yaml'
                all_files = [m.name for m in f.getmembers() if m.name != 'client/entrypoint']
                for member in all_files:
                    logger.info(member)
                # Extract all files except 'fedn.yaml'
                f.extractall(members=all_files)
                logger.info("[PRINT! Network]Successfully extracted compute package content in {}".format(self.dir))
                return True
